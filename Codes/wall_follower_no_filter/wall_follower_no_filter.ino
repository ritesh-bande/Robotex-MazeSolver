/* Wall Follower (quadrature encoders) — explicit rule-based navigation
   ...
   - Rule order (first match wins):
       ...
       2) Front blocked, tracked side still walled -> turn 90 right
          Front blocked, tracked side open          -> brief backoff, curve into it
   ...
   - No smoothing filter — reads raw sensor values directly, since the
     sensors were confirmed to already read accurately.
   - Wall-presence flags use hysteresis (wallHysteresis/frontHysteresis)
     so a reading sitting right at a threshold can't flap true/false
     every other loop cycle from ordinary sensor jitter.
   - PID state (pidInt/pidPrev) is reset on every path except the wall-
     following PID itself, so re-acquiring a wall after a turn/arc never
     computes a derivative against a stale, unrelated previous error.
...
const int wallHysteresis  = 10; // mm — dead zone to prevent rapid true/false flapping near wallThresh
const int frontHysteresis = 10; // mm — same idea for frontThresh

// Latched wall-presence flags — updated once per cycle by updateWallFlags().
// Using hysteresis instead of a single hard cutoff so ordinary sensor jitter
// right at the threshold can't flip a rule on and off every other loop.
...
void setupSensors() {
  ...
  // this bot runs at (targetDist=45mm, frontThresh=150mm).
...
void behaviorStep() {
  ...
  // 1) Dead end — walls on all three sides. Turn 90 right, then back off a
  //    little so the next cycle isn't immediately re-triggering this again
  //    if the new heading is also tight against something.
  ...
  // 2) Front blocked.
  //    If the TRACKED side happens to be open, back off briefly then curve
  //    into it instead of pivoting right — smoother, and avoids turning
  //    toward a wall that's still there on the other side when there was
  //    a free way out on the tracked side.
  if (wallFrontPresent) {
    bool trackedSideOpen = followLeft ? !wallLeftPresent : !wallRightPresent;

    if (trackedSideOpen) {
      mspeed(-60, -60); // brief, gentle backoff for margin before curving
      delay(15);
      if (followLeft) mspeed(innerSpeed, outerSpeed);  // curve left into the opening
      else             mspeed(outerSpeed, innerSpeed); // curve right into the opening
      pidInt = 0;
      pidPrev = 0;
      return;
    }
