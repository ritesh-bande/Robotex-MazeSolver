/* Wall Follower (quadrature encoders) — smooth-front-arc behavior
   - BTN1 = left wall follow
   - BTN2 = right wall follow
   - Smooth "search toward missing wall" logic (highest priority)
   - Front-obstacle -> brief backup + smooth in-place turn
   - Uses VL53L1X left/front/right, continuous non-blocking reads
   - Wall-following via a single mirrored PID function
...
const int baseSpeed    = 230;   // cruise speed while wall-following
const int targetDist   = 80;    // mm, desired wall distance
const int frontThresh  = 150;   // mm, below this = front obstacle
const int wallThresh   = 180;   // mm, above this = wall considered missing
...
const int rotationSpeed = 180;  // in-place turn cruise speed (now actually used, see rotate())
...
// Used only when there is NO wall on either side and nothing ahead —
...
void behaviorStep() {
  ...
  bool frontClear       = filtF >= frontThresh;

  // 0) Genuine open space — no wall on either side, nothing ahead.
  //    There's nothing to arc toward here, so go straight and let the
  //    encoders correct any drift instead.
  if (!wallLeftPresent && !wallRightPresent && frontClear) {
    driveStraightUsingEncoders();
    return;
  }

  // 1) Handle missing wall next — search behavior (higher priority than
  //    front obstacle, since a wall that's dropped away matters more
  //    than what's ahead)
  if (followLeft) {
    if (filtL > wallThresh) {
  ...
  // 2) Front obstacle — only relevant once we know we have a wall to follow
  if (filtF < frontThresh) {
    mspeed(-100, -100); // brief backup, not a full stop, to keep it fluid
    delay(20);

    if (followLeft) rotate(90);   // turn away from the left wall
    else             rotate(-90); // turn away from the right wall
    return;
  }

  // 3) Normal case — wall following with PID
  if (followLeft) {
